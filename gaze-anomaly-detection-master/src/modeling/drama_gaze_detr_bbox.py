...
class DRAMAGAZEDetr(torch.nn.Module):
    def __init__(self, args):
        super(DRAMAGAZEDetr, self).__init__()
        # self.config = config
        ...
        self.gaze_detr_encoder = copy.deepcopy(self.detr_encoder)

        self.img_feature_dim = int(args.img_feature_dim//2)
        self.latent_feat_size = detr_args.hidden_dim
        self.fc_1 = torch.nn.Linear(self.latent_feat_size, self.img_feature_dim)
        self.fc_2 = torch.nn.Linear(self.latent_feat_size, self.img_feature_dim)
    ...
    def forward(self, *args, **kwargs):
        ...
        gazes = kwargs['gaze_feats']
        B, S, C, H, W = images.shape  # batch, segment, chanel, hight, width

        images = images[:,0,...]
        gazes = gazes[:,0,...]

        raw_vid_feats = self.detr_encoder(images)
        gaze_feats = self.gaze_detr_encoder(gazes)

        raw_vid_feats = raw_vid_feats.view(B, self.latent_feat_size, -1).permute(0, 2, 1)
        gaze_feats = gaze_feats.view(B, self.latent_feat_size, -1).permute(0, 2, 1)

        raw_vid_feats = self.fc_1(raw_vid_feats)
        gaze_feats = self.fc_2(gaze_feats)

        vid_feats = torch.cat((raw_vid_feats, gaze_feats), dim=-1)
