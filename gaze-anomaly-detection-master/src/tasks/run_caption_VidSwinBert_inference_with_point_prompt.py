...
def inference(args, video_path, model, tokenizer, tensorizer):
    ...
    model.float()
    model.eval()
    frames = [np.array(Image.open(video_path)), np.array(Image.open(video_path))]
    frames = torch.tensor(frames).permute(0, 3, 1, 2)

    preproc_frames = _transforms(args, frames)
    data_sample = tensorizer.tensorize_example_e2e('', preproc_frames, text_b=None)
    data_sample = tuple(t.to(args.device) for t in data_sample)

    bbox = torch.tensor([0.612056, 0.544737, 0.66679, 0.801316]).to(args.device).to(data_sample[3].dtype)
