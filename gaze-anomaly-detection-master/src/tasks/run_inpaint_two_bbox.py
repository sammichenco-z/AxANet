...
def train(args, train_dataloader, val_dataloader, model, tokenizer, training_saver, optimizer, scheduler):
    ...
    for iteration, (img_keys, batch, meta_data) in enumerate(train_dataloader):
        ...
        # inpainted_frames = meta_data['inpainted_frames'].to(args.device)
        # inpainted_bbox = meta_data['inpainted_bbox'].to(args.device)
        model.train()
        # img_feats (B, #F, C, W, H)
        inputs = {
            ...
            'bbox': batch[6],
            # 'inpaint_feats': inpainted_frames,'inpaint_bboxes': inpainted_bbox
        }
        ...
        if args.pred_bbox:
            logits_bbox = outputs[-2]
            iou_bbox = outputs[-3]
            loss_bbox = outputs[-4]
            loss = loss + (loss_bbox * args.loss_bbox_w)
        if args.inpainted_pred_bbox:
            logits_bbox = outputs[-2]
            iou_bbox = outputs[-3]
            loss_bbox = outputs[-4]
            loss = loss + (loss_bbox.sum() * args.loss_bbox_w)
        ...
        if args.pred_bbox:
            ...
            # loss_dict['minor_loss'] = minor_loss

        if args.inpainted_pred_bbox:
            for i, level_bbox_loss in enumerate(loss_bbox):
                loss_dict[f'loss_bbox_{i}'] = level_bbox_loss
            loss_dict[f'loss_bbox'] = loss_bbox.sum()
        ...
        if backward_now:
            ...
            if args.pred_bbox:
                ...
                # TB_LOGGER.add_scalar('train/ori_loss', ori_loss.cpu(), global_step)
                # TB_LOGGER.add_scalar('train/minor_loss', minor_loss.cpu(), global_step)
            if args.inpainted_pred_bbox:
                
                for i, level_bbox_loss in enumerate(loss_bbox):
                    TB_LOGGER.add_scalar(f'train/loss_bbox_{i}', level_bbox_loss.cpu(), global_step)
                TB_LOGGER.add_scalar('train/loss_bbox', loss_bbox.sum().cpu(), global_step)
                    ...
                    if is_main_process():
                        if args.inpainted_pred_bbox:
                            with open(evaluate_file, 'r') as f:
                                ...
                                try:
                                    raw_eval_iou = tmp_data['raw_bbox_mean_iou']
                                    inpaint_eval_iou = tmp_data['inpaint_bbox_mean_iou']
                                except:
                                    print(tmp_data)
                                    raise Exception("json read error")
                            val_log = {f'valid/raw_bbox_mean_iou': raw_eval_iou}
                            val_log = {f'valid/inpaint_eval_iou': inpaint_eval_iou}
                            TB_LOGGER.log_scalar_dict(val_log)
                            aml_run.log(name='raw_bbox_mean_iou', value=float(raw_eval_iou))
                            aml_run.log(name='inpaint_eval_iou', value=float(inpaint_eval_iou))
...
def evaluate(args, val_dataloader, model, tokenizer, output_dir):
    ...
    if is_main_process():
        ...
        iou_result = 0.
        if args.inpainted_pred_bbox:
            iou_result = level_bbox_eval(predict_file, outfile=bbox_eval_file)
        elif args.pred_bbox:
        ...
        with torch.no_grad():
            for step, (img_keys, batch,meta_data) in tqdm(enumerate(test_dataloader)):
                ...
                inputs = {'is_decode': True,
                    ...
                    'masked_pos': batch[4],
                    'bbox': batch[5],
                    # 'inpaint_feats': inpainted_frames,
                    # 'inpaint_bboxes': inpainted_bbox,
                    'do_sample': False,
