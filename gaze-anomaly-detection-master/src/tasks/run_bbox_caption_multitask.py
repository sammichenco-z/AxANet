...
from src.evalcap.utils_caption_evaluate import evaluate_on_coco_caption, two_cap_evaluate_on_coco_caption, bbox_eval
...
from src.modeling.drama_e2e_vid_swin_bert import DRAMAVideoTransformerMultitask
...
def mixed_precision_init(args, model):
    ...
    decay_heads_param_tp = [(n, p) for n, p in decay_param_tp if "swin." not in n]

    no_decay_swin_param_tp = [(n, p) for n, p in no_decay_param_tp if "swin." in n]
    no_decay_heads_param_tp = [(n, p) for n, p in no_decay_param_tp if "swin." not in n]

    decay_bert_param_tp = [(n, p) for n, p in decay_heads_param_tp if "bbox_head." not in n]
    no_decay_bert_param_tp = [(n, p) for n, p in no_decay_heads_param_tp if "bbox_head." not in n]

    # accelate bounding box prediction
    decay_bbox_param_tp = [(n, p) for n, p in decay_heads_param_tp if "bbox_head." in n]
    no_decay_bbox_param_tp = [(n, p) for n, p in no_decay_heads_param_tp if "bbox_head." in n]


    weight_decay = 0.2
    coef_lr = args.backbone_coef_lr 
    bbox_head_lr = args.bbox_head_lr 
    optimizer_grouped_parameters = [
        ...
        {'params': [p for n, p in decay_bert_param_tp], 
            'weight_decay': weight_decay},
        {'params': [p for n, p in decay_bbox_param_tp], 
            'weight_decay': weight_decay,
            'lr': args.learning_rate * bbox_head_lr},
        {'params': [p for n, p in no_decay_swin_param_tp],
            'weight_decay': 0.0,
            'lr': args.learning_rate * coef_lr},
        {'params': [p for n, p in no_decay_bert_param_tp], 
            'weight_decay': 0.0},
        {'params': [p for n, p in no_decay_bbox_param_tp], 
            'weight_decay': 0.0,
            'lr': args.learning_rate * bbox_head_lr}
    ]
...
def train(args, train_dataloader, val_dataloader, model, tokenizer, training_saver, optimizer, scheduler):
    ...
    for iteration, (img_keys, batch, meta_data) in enumerate(train_dataloader):
        ...
        if args.mixed_precision_method == "fairscale":
            ...
            outputs = model(**inputs)
        loss, logits = outputs['text_loss'], outputs['text_logits']
        ...
        if args.multitask:
            ...
            loss = loss + (loss_bbox * args.loss_bbox_w + loss_class * args.loss_class_w)
    ...
    def gen_rows():
        ...
        with torch.no_grad():
            for step, (img_keys, batch, meta_data) in tqdm(enumerate(test_dataloader)):
                ...
                all_caps = outputs['pred_caps']  # batch_size * num_keep_best * max_len
                all_confs = torch.exp(outputs['pred_scores'])
                ...
                for img_key, caps, confs, box, box_iou, class_pred, class_gt in zip(img_keys, all_caps, all_confs, all_bboxs, all_bbox_ious, all_classes, all_gt_classes):
                    res = []
...
def get_custom_args(base_config):
    ...
    parser.add_argument('--bbox_head_lr', type=float, default=0.001)
...
def main(args):
    ...
    if args.multitask:
        vl_transformer = DRAMAVideoTransformerMultitask(args, config, swin_model, bert_model)
    else:
