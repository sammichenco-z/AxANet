...
def main(args):
    ...
    print("Number of parameter: %.2fM" % (total_parameters/(1024*1024)))
    return
    ...
    # raw_video_path = "ADA/fig4_choose/my_choose"
    # root_path =  "ADA/fig4_choose/my_choose"
    ...
    for mode in ['test']:
        ...
        for idx in tqdm(range(len(filenames))):
            ...
            save_path = "grad_cam_human_machine_final_version_driver_no_pretrained_no_finetune"
            ...
            for i in range(len(grayscale_cam)):
                ...
                cv2.rectangle(cam_image, (int(all_bboxs[0]*w), int(all_bboxs[1]*h)), (int(all_bboxs[2]*w), int(all_bboxs[3]*h)), (0, 0, 0), 4)
                os.makedirs(save_path+"_with_bbox", exist_ok=True)
