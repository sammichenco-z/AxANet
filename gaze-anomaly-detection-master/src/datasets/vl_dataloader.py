...
from .anomaly_dataset import ANOMALY_DATASET, ANOMALY_GAZE_DATASET
...
def drama_dataset(args, tokenizer, tensorizer, mode='train', is_train=True, use_gaze=False):
    ...
    if mode == 'unlabeled':
        ...
        # anomaly only change this
        return ANOMALY_GAZE_DATASET(args, dataset_dir, tokenizer, tensorizer, is_train, mode)
        ...
        # anomaly only change this
        return ANOMALY_DATASET(args, dataset_dir, tokenizer, tensorizer, is_train, mode)

def build_dataset(args, tokenizer, mode='train', is_train=True, use_gaze=True):
    tensorizer = build_tensorizer(args, tokenizer, is_train=is_train)
    return drama_dataset(args, tokenizer, tensorizer, mode=mode, is_train=is_train, use_gaze=use_gaze)
...
def make_data_loader(args, tokenizer, is_distributed=True, mode='train',
        is_train=True, start_iter=0, num_gpus=8, use_gaze=True):

    dataset = build_dataset(args, tokenizer, mode=mode, is_train=is_train, use_gaze=use_gaze)
