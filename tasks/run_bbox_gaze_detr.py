...
from src.modeling.drama_gaze_detr_bbox import DRAMAGAZEDetr
...
def mixed_precision_init(args, model):#最大精度
    ...
    decay_swin_param_tp = [(n, p) for n, p in decay_param_tp if "detr_encoder.backbone." in n or "gaze_detr_encoder.backbone." in n]
    decay_bert_param_tp = [(n, p) for n, p in decay_param_tp if "detr_encoder.backbone." not in n and "gaze_detr_encoder.backbone." not in n]

    no_decay_swin_param_tp = [(n, p) for n, p in no_decay_param_tp if "detr_encoder.backbone." in n or "gaze_detr_encoder.backbone." in n]
    no_decay_bert_param_tp = [(n, p) for n, p in no_decay_param_tp if "detr_encoder.backbone." not in n and "gaze_detr_encoder.backbone." not in n]
...
def main(args):
    ...
    if args.pred_bbox:
        vl_transformer = DRAMAGAZEDetr(args)
        ...
        # print(vl_transformer.gaze_detr_encoder.load_state_dict(detr_pretrained_model['model'], strict=False))
    ...
    if args.do_train:
        ...
        train_dataloader = make_data_loader(args, tokenizer, args.distributed, mode='train', is_train=True, use_gaze=True)
        val_dataloader = make_data_loader(args, tokenizer, args.distributed, mode='test', is_train=False, use_gaze=True)
