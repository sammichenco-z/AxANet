...
def video_bbox_prcoess(is_train, img_res, seed=None):
    if is_train:
        return Compose([
...
class RandomSelect(object):
    ...
    def __call__(self, img, bbox):
        if random.random() < self.p:
            return self.transforms1(img, bbox)
        return self.transforms2(img, bbox)
        ...
        if random.random() < self.p:
            if bbox is not None:
                if isinstance(bbox, list):
                    bbox = bbox[:, [2, 1, 0, 3]] * torch.as_tensor([-1, 1, -1, 1]) + torch.as_tensor([1, 0, 1, 0])
                else:
                    bbox_to_check = torch.as_tensor([1-bbox[2], bbox[1], 1-bbox[0], bbox[3]])
                    bbox = bbox[[2, 1, 0, 3]] * torch.as_tensor([-1, 1, -1, 1]) + torch.as_tensor([1, 0, 1, 0])
                    assert (bbox_to_check == bbox).all()
...
class RandomVerticalFlip(object):
    ...
    def __call__(self, clip, bbox=None):
        ...
        if random.random() < self.p:
            ...
            if bbox is not None:
                if isinstance(bbox, list):
                    bbox = bbox[:, [0, 3, 2, 1]] * torch.as_tensor([1, -1, 1, -1]) + torch.as_tensor([0, 1, 0, 1])
                else:
                    bbox_to_check = torch.as_tensor([bbox[0], 1-bbox[3], bbox[2], 1-bbox[1]])
                    bbox = bbox[[0, 3, 2, 1]] * torch.as_tensor([1, -1, 1, -1]) + torch.as_tensor([0, 1, 0, 1])
                    assert (bbox_to_check == bbox).all()
    ...
    def __call__(self, clip, bbox=None):
        ...
        if bbox is not None:
            if isinstance(bbox, list):
                ...
                bbox = cropped_boxes.reshape(-1, 4)
