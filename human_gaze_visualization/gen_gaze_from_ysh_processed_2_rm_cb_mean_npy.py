...
parser.add_argument("--root_time", type=str, default="10_18")
parser.add_argument("--split", type=str, default="old")
parser.add_argument("--gaussian", type=int, default=20)
...
if True:
    ...
    # for begin_rate, end_rate in [(0.1,0.9)]:
        ...
        for image_path_task in tqdm(average_result.keys()):
            ...
            # with open(average_result[image_path_task+"save"].replace(split, f"{split}_mean")+'.json', "w") as f:
            #     json_gaze_data = {
            #         "gaze_data": average_result[image_path_task]
            #     }
            #     json.dump(json_gaze_data, f)

            tmp_save_path = average_result[image_path_task+"save"].replace(split, f"{split}_mean")
            os.makedirs(os.path.dirname(tmp_save_path), exist_ok=True)
            heatmap = draw_heatmap(average_result[image_path_task], (display_size, display_size), alpha=0.5, savefilename=tmp_save_path, imagefile=image_path_task.split("**")[0], gaussianwh=gaussianwh, gaussiansd=gaussiansd)
            np.save(tmp_save_path+'.npy', heatmap)
