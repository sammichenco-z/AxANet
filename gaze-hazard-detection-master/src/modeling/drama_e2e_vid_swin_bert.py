...
class DRAMAVideoTransformerMultitask(torch.nn.Module):
    def __init__(self, args, config, swin, transformer_encoder):
        super(DRAMAVideoTransformerMultitask, self).__init__()
        self.config = config
    ...
    def forward(self, *args, **kwargs):
        ...
        bbox_gt = kwargs.pop('bbox')
        bbox_loss = get_bbox_loss(bbox_logits, bbox_gt)        
        with torch.no_grad():
            bbox_iou = get_iou(bbox_logits, bbox_gt)

        class_gt = kwargs.pop('label')
        class_loss = get_class_loss(class_logits, class_gt)
