...
def train(args, train_dataloader, val_dataloader, model, tokenizer, training_saver, optimizer, scheduler):
    ...
    if args.text_embedding_type == "clip":
        text_encoder, _ = clip.load("ViT-B/32")
        text_encoder = text_encoder.to(args.device)
    elif args.text_embedding_type == "blip2" or args.text_embedding_type == "blip2_multimodal":
        blip2_model, vis_processors, txt_processors = load_model_and_preprocess(name="blip2_feature_extractor", model_type="pretrain", is_eval=True, device=args.device)
    else:
        raise Exception("Error: text_embedding pretrained not implented.")
    ...
    for iteration, (img_keys, batch, meta_data) in enumerate(train_dataloader):
        ...
        with torch.no_grad():
            if args.text_embedding_type == "clip":
                tokenized_caption = clip.tokenize(meta_data['caption']).to(args.device)
                text_feature = text_encoder.encode_text(tokenized_caption)
            elif args.text_embedding_type == "blip2":
                processed_image = None
                processed_caption = [txt_processors["eval"](cap) for cap in meta_data['caption']]
                text_feature = blip2_model.extract_features({'image': processed_image, 'text_input': processed_caption}, mode="text").text_embeds[:,:1,:]
            elif args.text_embedding_type == "blip2_multimodal":
                processed_image = [vis_processors["eval"](Image.fromarray(raw_img.permute(1,2,0).numpy())) for raw_img in meta_data['raw_image']]
                processed_image = torch.stack(processed_image, dim=0).to(torch.float16).to(args.device)
                processed_caption = [txt_processors["eval"](cap) for cap in meta_data['caption']]
                text_feature = blip2_model.extract_features({'image': processed_image, 'text_input': processed_caption}).multimodal_embeds
                text_feature = text_feature[:,:1,:].mean(1).unsqueeze(1)
        ...
        inputs = {
            ...
            'bbox': batch[6],
            'label': batch[7],
            'caption': text_feature, # not tokenized
        }
        ...
        if args.pred_bbox_with_clip_caption:
            bbox_logits = outputs["bbox_logits"]
            loss_bbox = outputs["bbox_loss"]
            bbox_iou = outputs["bbox_iou"]
            loss = loss_bbox * args.loss_bbox_w

            if "distillation_loss" in outputs.keys():
        ...
        if backward_now:
            ...
            if args.pred_bbox_with_clip_caption:
                ...
                TB_LOGGER.add_scalar('train/distillation_loss', distillation_loss.cpu(), global_step)
    ...
    def gen_rows():
        ...
        with torch.no_grad():
            for step, (img_keys, batch, meta_data) in tqdm(enumerate(test_dataloader)):
                ...
                #         processed_caption = [txt_processors["eval"](cap) for cap in meta_data['caption']]
                #         text_feature = blip2_model.extract_features({'image': processed_image, 'text_input': processed_caption}).multimodal_embeds
                #         text_feature = text_feature.mean(1).unsqueeze(1)
                #         # text_feature = text_feature[:,:1,:]
                text_feature = None
                ...
                if not args.use_sep_cap:
                    for img_key, box, box_iou, class_gt in zip(img_keys, all_bboxs, all_bbox_ious, all_gt_classes):
                        res = []
...
def get_custom_args(base_config):
    ...
    parser.add_argument('--text_embedding_type', type=str, default='clip',
                       choices=['clip', 'blip2', 'blip2_multimodal'],
