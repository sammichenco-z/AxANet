...
class BDDOIA_dataset(object):
    ...
    def __getitem__(self, idx):
        ...
        new_frames = np.stack([raw_frames[0], gaze_frames[0]])
        ...
        if self.detr_aug:
            ...
            preproc_frames = self.apply_bbox_augmentations(new_frames, seed=my_seed_for_this_iter, is_gaze=False)
        else:
            # preproc_frames = self.apply_augmentations(raw_frames)
            # preproc_gaze_frames = self.apply_augmentations(gaze_frames)
            preproc_frames = self.apply_augmentations(new_frames)

        preproc_gaze_frames = torch.tensor(1)
        # preparing outputs
        meta_data = {}
