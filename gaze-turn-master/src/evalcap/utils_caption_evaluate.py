...
def class_eval(res_file, outfile=None):
    ...
    acc_l = accuracy_score(l_label_list,l_pred_list)
    precision_l = precision_score(l_label_list,l_pred_list)
    recall_l = recall_score(l_label_list,l_pred_list)
    print(f1_l, acc_l, precision_l, recall_l)
    ...
    if not outfile:
        ...
        with open(outfile, 'w') as fp:
            json.dump({'f1_reason_all': f1_reason_all,'f1_action_all':f1_action_all,'f1_total_all':f1_total_all,
                       ...
                       'mf1_action': mf1_action, 'mf1_reason': mf1_reason, "acc_l": acc_l}, fp, indent=4)
...
if __name__=='__main__':
    file='/usr/aidrive1/drama/output/oia_new_4/checkpoint-100-198600/pred.test.beam1.max30.tsv'
