...
class myModelDecoder(nn.Module):
    def __init__(self, latent_feat_size, num_queries=1, aux_loss=False):
        ...
        self.class_embed = nn.Linear(hidden_dim * 2, (4+21)*2)
...
class myTrEncoder(nn.Module):
    ...
    def forward(self, src, mask, pos_embed, 
        ...
        memory_img = memory[:memory.shape[0] // 2,...]
        memory_gaze = memory[memory.shape[0] // 2:,...]

        memory_img = memory_img.mean(0)
        memory_gaze = memory_gaze.mean(0)

        memory = torch.cat([memory_img, memory_gaze], dim=1)
