...
class myModelDecoder(nn.Module):
    def __init__(self, latent_feat_size, num_queries=2, aux_loss=False):
        super().__init__()
        ...
        self.action_embed = nn.Linear(hidden_dim, 4*2)
        self.reason_embed = nn.Linear(hidden_dim, 21*2)
    ...
    def forward(self, vid_feats, gaze_feats):
        ...
        pred_action = self.action_embed(hs[0])
        pred_action = pred_action.view(pred_action.shape[0], -1, 2)
        pred_reason = self.reason_embed(hs[1])
        pred_reason = pred_reason.view(pred_reason.shape[0], -1, 2)
        outputs_class = torch.cat([pred_action, pred_reason], dim=1)
...
class myTrEncoder(nn.Module):
    ...
    def forward(self, src, mask, pos_embed, 
        ...
        src = torch.cat([tgt, src, src_gaze], dim=0)
        pos_embed = torch.cat([query_embed, pos_embed, pos_embed_gaze], dim=0)
        mask = torch.cat([query_mask, mask, mask_gaze], dim=1)

        memory = self.encoder(src, src_key_padding_mask=mask, pos=pos_embed) # 98 32 256

        memory = memory[:query_embed.shape[0],...]
