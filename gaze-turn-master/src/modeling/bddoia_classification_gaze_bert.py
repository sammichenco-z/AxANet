...
from src.modeling.load_action_pred_head import get_gaze_bbox_pred_model
...
class DRAMAGAZEVideoTransformer_gaze(torch.nn.Module):
    def __init__(self, args, config, swin_model_rgb, swin_model_gaze, transformer_encoder):
        ...
        # to change the dim of video token and text token
        if self.token_channel_concat:
            self.fc_rgb = torch.nn.Linear(self.latent_feat_size, self.img_feature_dim//2)
            self.fc_gaze = torch.nn.Linear(self.latent_feat_size, self.img_feature_dim//2)
        else:
        ...
        self.gaze_action_head = get_gaze_bbox_pred_model(args)
    ...
    def forward(self, *args, **kwargs):
        ...
        all_vid_feats = torch.cat((vid_feats, gaze_feats), dim=-1)
        ...
        logits_action, logits_reason, loss_action, loss_reason, attn_layers = self.gaze_action_head(all_vid_feats, kwargs['label_action'], kwargs['label_reason'])
