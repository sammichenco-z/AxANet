...
class myModelDecoder(nn.Module):
    def __init__(self, args, latent_feat_size, num_queries=1, aux_loss=False):
        super().__init__()
        self.args = args
        ...
        self.transformer_decoder = myTrEncoder(args, hidden_dim) # todo
        
        self.class_embed = nn.Linear(hidden_dim, (4+21)*2)
        
        self.query_embed = nn.Embedding(num_queries, hidden_dim)
        self.latent_feat_size = latent_feat_size
        self.input_proj_vid = nn.Conv2d(self.latent_feat_size, hidden_dim // 2, kernel_size=1)
        self.input_proj_gaze = nn.Conv2d(self.latent_feat_size+16, hidden_dim // 2, kernel_size=1)
    ...
    def forward(self, vid_feats, gaze_feats):
        ...
        vid_feats = torch.cat([vid_feats, gaze_feats], dim=1)
...
class myTrEncoder(nn.Module):
    ...
    def __init__(self, args, d_model=256, nhead=8, num_encoder_layers=6,
                 num_decoder_layers=6, dim_feedforward=2048, dropout=0.1,
                 activation="relu", normalize_before=False,
                 return_intermediate_dec=False):
        super().__init__()
