...
class DRAMAGAZEVideoTransformer_gaze(torch.nn.Module):
    def __init__(self, args, config, swin_model_rgb, swin_model_gaze, transformer_encoder):
        ...
        self.classifier = nn.Sequential(
            nn.Linear(self.latent_feat_size*2, (4+21)*2)
        )
        # self.classifier = nn.Sequential(
        #     nn.Linear(self.latent_feat_size*49*2, 1024),
        #     nn.ReLU(),
    ...
    def forward(self, *args, **kwargs):
        ...
        # vid_feats = self.fc_rgb(vid_feats)
        ...
        gaze_feats = gaze_feats.view(B, -1, self.latent_feat_size)
        # gaze_feats = self.fc_gaze(gaze_feats)


        # bbox head
        vid_feats = vid_feats.mean(1)
        gaze_feats = gaze_feats.mean(1)
        
        # vid_feats = vid_feats.view(B, -1)        
        # gaze_feats = gaze_feats.view(B, -1)        
        all_feats = torch.cat((vid_feats, gaze_feats), dim=-1)
        # all_feats = all_feats.view(B, -1)
        
        logits = self.classifier(all_feats)
        logits = logits.view(B, -1, 2) # (B, 25, 2)
        logits_action = logits[:, :4, :]
        logits_reason = logits[:, 4:, :]
        #logits_test = logits[:,0,:]

        #preds_action = torch.argmax(logits_action, dim=-1)
        #preds_reason = torch.argmax(logits_reason, dim=-1)
        
        # pred = torch.softmax(logits_action, dim=-1)
        
        #loss_test = self.get_cross_loss(logits_action.view(-1, 2), kwargs['label_action'].view(-1))
        ...
        #loss_test=self.get_l1_loss(pred,kwargs['label_action'][:,3])
        #dict={'pred':(torch.argmax(logits_action, dim=-1)),'label':kwargs['label_action']}
        #print(dict)
        #print(loss_test)
        return (logits_action, logits_reason, loss_action, loss_reason)
        #return logits_action,loss_test
