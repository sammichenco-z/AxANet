...
class myModelDecoder(nn.Module):
    ...
    def forward(self, vid_feats, gaze_feats):
        ...
        if gaze_feats is not None:
            ...
            gaze_masks = torch.zeros([gaze_feats.shape[0], gaze_feats.shape[2], gaze_feats.shape[3]], dtype=torch.bool, device=vid_feats.device)
            gaze_pos = self.position_encoding(gaze_feats, gaze_masks).to(torch.float16)
        else:
            gaze_feats = None
            gaze_masks = None
            gaze_pos = None
