...
from src.modeling.drama_gaze_bbox import DRAMAGAZEVideoTransformer
...
def train(args, train_dataloader, val_dataloader, model, tokenizer, training_saver, optimizer, scheduler):
    ...
    for iteration, (img_keys, batch, meta_data) in enumerate(train_dataloader):
        ...
        gaze_data = meta_data['gaze'].to(args.device)
        model.train()
        # img_feats (B, #F, C, W, H)
        inputs = {
        ...
        if args.pred_bbox or args.gaze_pred_bbox:
                ...
                if args.evaluate_during_training:
                    ...
                    if is_main_process():
                        with open(evaluate_file, 'r') as f:
                            eval_iou = json.load(f)['bbox_mean_iou']
                        val_log = {f'valid/bbox_iou': eval_iou}
                        TB_LOGGER.log_scalar_dict(val_log)
                        aml_run.log(name='iou', value=float(eval_iou))                
                    #     if args.use_sep_cap:
            ...
            for step, (img_keys, batch, meta_data) in tqdm(enumerate(test_dataloader)):
                ...
                if not args.use_sep_cap:
                    for img_key, box, box_iou in zip(img_keys, all_bboxs, all_bbox_ious):
                        res = []
                        res.append({'box': str(box.cpu()), 'box_iou': str(box_iou.cpu().item())})
...
def main(args):
    ...
    elif args.gaze_pred_bbox:
        vl_transformer = DRAMAGAZEVideoTransformer(args, config, swin_model, bert_model)
    else:
