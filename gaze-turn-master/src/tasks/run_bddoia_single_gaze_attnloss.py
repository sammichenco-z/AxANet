...
from src.modeling.bddoia_classification_gaze_selfattn_attncenterloss import DRAMAGAZEVideoTransformer_gaze
...
def train(args, train_dataloader, val_dataloader, model, tokenizer, training_saver, optimizer, scheduler):
    ...
    for iteration, (img_keys, batch, meta_data) in enumerate(train_dataloader):
        ...
        if args.bddoia:
            logits_action, logits_reason, loss_action, loss_reason, loss_attn = outputs[:]
            loss =  loss_action + loss_reason + loss_attn
        ...
        if backward_now:
            ...
            if args.bddoia:
                ...
                TB_LOGGER.add_scalar('train/loss_attn', loss_attn.cpu(), global_step)
        ...
        with torch.no_grad():
            for step, (img_keys, batch, meta_data) in tqdm(enumerate(test_dataloader)):
                ...
                if args.visualize_attn:
                    logits_action, logits_reason, loss_action, loss_reason, loss_attn, attn_layers = outputs[:]
