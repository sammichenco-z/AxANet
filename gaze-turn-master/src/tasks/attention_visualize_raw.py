    ...
    if True:
        ...
        if True:
            for step, (img_keys, batch, meta_data) in tqdm(enumerate(test_dataloader)):
                ...
                logits_action, logits_reason, loss_action, loss_reason, attentions = outputs[:]

                # grayscale_cam = rollout(attentions, 0.9, 'mean')

                # grayscale_cam = attentions

                layer_number = len(attentions)

                for layer_id in range(layer_number):
                    raw_attention = attentions[layer_id][:1, 2, :]
                    raw_imgs = meta_data['raw_frame'][:, -1, :, :, :]
                    raw_imgs = raw_imgs.permute(0, 2, 3, 1).numpy()
                    h, w = raw_imgs.shape[1], raw_imgs.shape[2]
                    ...
                    cam_img = cv2.resize(raw_attention, (w, h))


                    cam_image = show_cam_on_image(raw_imgs[0] / 255, cam_img).copy()

                    cv2.putText(cam_image, str(preds_action[0][2].item()) + "  " + str(labels_action[0][2].item()), (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)

                    save_path = f"raw_attention_layer{layer_id}"
                    os.makedirs(save_path, exist_ok=True)
                    cv2.imwrite(os.path.join(save_path, img_keys[0]+"_"+str(0).zfill(3)+".jpg"), cam_image)
