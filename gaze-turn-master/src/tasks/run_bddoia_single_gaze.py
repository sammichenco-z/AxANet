...
# from src.datasets.bddoia_singleframe_dataloader_gaze_dotinput import make_data_loader
...
# from src.modeling.bddoia_classification_gaze import DRAMAGAZEVideoTransformer_gaze
from src.modeling.bddoia_classification_gaze_selfattn import DRAMAGAZEVideoTransformer_gaze
# from src.modeling.bddoia_classification_gaze_selfattn_sublearnprior import DRAMAGAZEVideoTransformer_gaze
# from src.modeling.bddoia_classification_gaze_selfattn_concatgprior import DRAMAGAZEVideoTransformer_gaze
# from src.modeling.bddoia_classification_gaze_bert import DRAMAGAZEVideoTransformer_gaze
# from src.modeling.bddoia_classification_gaze_selfattn_smallgazebkbn import DRAMAGAZEVideoTransformer_gaze
...
def mixed_precision_init(args, model):#最大精度
    ...
    decay_swin_param_tp = [(n, p) for n, p in decay_param_tp if "swin_" in n]
    decay_bert_param_tp = [(n, p) for n, p in decay_param_tp if "swin_" not in n]

    no_decay_swin_param_tp = [(n, p) for n, p in no_decay_param_tp if "swin_" in n]
    no_decay_bert_param_tp = [(n, p) for n, p in no_decay_param_tp if "swin_" not in n]
...
def train(args, train_dataloader, val_dataloader, model, tokenizer, training_saver, optimizer, scheduler):
    ...
    for iteration, (img_keys, batch, meta_data) in enumerate(train_dataloader):
        ...
        inputs = {
            'img_feats': batch[0],
            'label_action': batch[1],
            'label_reason': batch[2],
            'gaze': batch[3],
        }
        ...
        if args.bddoia:
            logits_action, logits_reason, loss_action, loss_reason = outputs[:]
            loss =  loss_action + loss_reason
            #logits,loss_action = outputs[:]
            #loss=loss_action
        ...
        loss_dict['loss_reason'] = loss_reason
        ...
        if backward_now:
            ...
            if args.bddoia:
                TB_LOGGER.add_scalar('train/loss_action', loss_action.cpu(), global_step)
                TB_LOGGER.add_scalar('train/loss_reason', loss_reason.cpu(), global_step)
                #TB_LOGGER.add_scalar('train/loss_test',loss.cpu(),global_step)
        ...
        if backward_now:
            ...
            if (args.save_steps > 0 and global_step % args.save_steps == 0) or global_step == max_global_step:
                ...
                if args.evaluate_during_training:
                    ...
                    evaluate_file = evaluate(args, val_dataloader, model, tokenizer, checkpoint_dir,global_step)
                    if get_world_size() > 1:
                        dist.barrier()
                    if is_main_process():
                        with open(evaluate_file, 'r') as f:
                            #eval_mean_accuracy = json.load(f)['mean_accuracy']
                            eval_f1_total = json.load(f)['f1_action_all']
                        val_log = {f'valid/eval_mean_accuracy': eval_f1_total}
                        TB_LOGGER.log_scalar_dict(val_log)
                        aml_run.log(name='eval_f1_total', value=float(eval_f1_total))                
                        if (epoch > args.num_train_epochs * 0.9) and (epoch % 10 == 0):
                            training_saver.save_model(checkpoint_dir, global_step, model, optimizer)
                    #     if args.use_sep_cap:
        ...
        with torch.no_grad():
            for step, (img_keys, batch, meta_data) in tqdm(enumerate(test_dataloader)):
                ...
                if args.visualize_attn:
                    logits_action, logits_reason, loss_action, loss_reason, attn_layers = outputs[:]
                    attn_layers = torch.stack(attn_layers, dim=1)
                else:
                    logits_action, logits_reason, loss_action, loss_reason = outputs[:]
                    attn_layers = [None]*logits_action.shape[0]
                ...
                labels_action = batch[1]
                
                #for i in range(len(labels_action)):
                #    labels_action[i][0]=1
                labels_reason = batch[2]
                #label = labels_action[:,3]
                
                for img_key, pred_action, pred_reason, label_action, label_reason, attn_layers_item in zip(img_keys, preds_action, preds_reason, labels_action, labels_reason, attn_layers):
                    res = []
                    res.append({'pred_action': str(pred_action.cpu()), 'pred_reason': str(pred_reason.cpu()), 
                                'label_action': str(label_action.cpu()), 'label_reason': str(label_reason.cpu())})
                    
                    if args.visualize_attn:
                        attn_path = op.join(tmp_file.split('/')[0], tmp_file.split('/')[1], tmp_file.split('/')[2], 'attn_weight')
                        np.save(op.join(attn_path, img_key+".npy"), np.array(attn_layers_item.cpu()))
                ...
                #for img_key, pred, labels_action in zip(img_keys, pred, labels_action):
                #    res=[]
                #    res.append({'pred_action': str(pred.cpu()), 
                #             'label_action': str(labels_action.cpu())})
                #    if isinstance(img_key, torch.Tensor):
                #        img_key = img_key.item()
                #    yield img_key, json.dumps(res)
...
def main(args):
    ...
    swin_model_rgb = get_swin_model(args)
    swin_model_gaze = get_swin_model(args)
    # Get BERT and tokenizer 
    bert_model, config, tokenizer = get_bert_model(args)
    # build SwinBERT based on training configs
    if args.bddoia:
        vl_transformer = DRAMAGAZEVideoTransformer_gaze(args, config, swin_model_rgb, swin_model_gaze, bert_model)
    else:
        exit()
