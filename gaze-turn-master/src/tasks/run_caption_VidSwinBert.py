        ...
        with torch.no_grad():
            for step, (img_keys, batch, meta_data) in tqdm(enumerate(test_dataloader)):
                ...
                if not args.use_sep_cap:
                    for img_key, caps, confs, box, box_iou in zip(img_keys, all_caps, all_confs, all_bboxs, all_bbox_ious):
                        res = []
                        for cap, conf in zip(caps, confs):
                            cap = tokenizer.decode(cap.tolist(), skip_special_tokens=True)
                            res.append({'caption': cap, 'conf': conf.item(), 'box': str(box.cpu()), 'box_iou': str(box_iou.cpu().item())})
