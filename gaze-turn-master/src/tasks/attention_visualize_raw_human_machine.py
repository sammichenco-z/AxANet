...
def test(args, model, tokenizer, predict_file):
    ...
    if True:
        ...
        with open("drama_bbox_gt.json", "r") as f:
        ...
        if True:
            for step, filename in tqdm(enumerate(filenames)):
                ...
                logits_action, logits_reason, loss_action, loss_reason, raw_attentions = outputs[:]
                ...
                gt_key = filename
                if gt_key in action_gt:
                    labels_action[0][2] = int(action_gt[gt_key][2])
                    acc.append(int(action_gt[gt_key][2]) == preds_action[0][2].item())
                    if int(action_gt[gt_key][2]) != preds_action[0][2].item():
                        wrong_case.append((gt_key, int(action_gt[gt_key][2]), preds_action[0][2].item()))
                else:
                    labels_action[0][2] = -1
                ...
                # layer_number = len(raw_attentions)
                # head_number = len(raw_attentions[0][0])
                ...
                #         attentions = rollout(raw_attentions, 0.0, 'mean', layer_id, head_id)
                #         attention = attentions[2, ...]

                #         attention = attention-attention.min()
                #         attention = attention / attention.max()

                #         save_path = f"raw_attention_human_machine_detr_v3_{args.human_choose}_{args.time}"
                #         os.makedirs(f"{save_path}/{layer_id}/{head_id}", exist_ok=True)

                #         cam_img = attention
                #         # cam_img = cam_img/cam_img.max()
                #         cam_array = cam_img.copy()

                #         # np.save(f"{save_path}/{layer_id}/{filename}", cam_array)

                #         cam_img = cv2.resize(np.float32(cam_img), (w, h))
                #         head_mean.append(cam_img)

                #         cam_image = show_cam_on_image(raw_imgs / 255, cam_img).copy()

                #         cv2.putText(cam_image, "P: "+str(preds_action[0][2].item()) + "  G: " + str(labels_action[0][2].item()), (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)

                #         head_cams.append(cam_image)

                #         cv2.imwrite(f"{save_path}/{layer_id}/{head_id}/{filename}", cam_image)
                    
                #     head_cams = np.hstack(head_cams)
                #     cv2.imwrite(f"{save_path}/{layer_id}/head_cams_{filename}", head_cams)

                #     head_mean = np.mean(np.asarray(head_mean), 0)
                #     head_mean = head_mean - head_mean.min()
                #     head_mean = head_mean / head_mean.max()
                #     gead_mean_cam_image = show_cam_on_image(raw_imgs / 255, head_mean).copy()
                #     cv2.imwrite(f"{save_path}/{layer_id}/mean_{filename}", gead_mean_cam_image)         


        print(wrong_case)
        print(np.array(acc).mean())
        print(f"gt_len:{len(acc)}")
...
def update_existing_config_for_inference(args):
    ...
    train_args.human_choose = args.human_choose
...
def get_custom_args(base_config):
    ...
    parser.add_argument('--human_choose', type=str, default='None')
