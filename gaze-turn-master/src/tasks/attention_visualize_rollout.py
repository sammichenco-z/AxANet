    ...
    if True:
        ...
        if True:
            for step, (img_keys, batch, meta_data) in tqdm(enumerate(test_dataloader)):
                ...
                if True:
                    for action_id in range(grayscale_cam.shape[0]):

                        if labels_action[0][action_id] != 1 or preds_action[0][action_id] != 1:
                            continue

                        cam_img = grayscale_cam[action_id]
                        ...
                        save_path = "rollout_vis1_"+str(action_id)
