...
if True:
    for idx in tqdm(range(0, len(filenames))):
        ...
        with open(filename, "rb") as f:
            ...
            gaze_data = [(int((xyxy[0]+xyxy[2])/2*img_w), int((xyxy[1]+xyxy[3])/2*img_h), 100)]
            ...
            if os.path.exists(op.join(video_path, 'fake_saliency_my')) and os.path.exists(op.join(video_path, 'fake_saliency_my', img_name)):
                try:
                    if cv2.imread(op.join(video_path, 'fake_saliency_my', img_name)).shape == img_data.shape:
                        continue
                except:
                    pass
                
            os.makedirs(op.join(video_path, 'fake_saliency_my'), exist_ok=True)
            
            draw_heatmap(gaze_data, (img_w, img_h), alpha=alpha, savefilename=op.join(video_path, 'fake_saliency_my', img_name), imagefile=background_image, gaussianwh=ngaussian, gaussiansd=sd)
