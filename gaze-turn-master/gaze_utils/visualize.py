...
def draw_heatmap(gazepoints, dispsize, imagefile=None, alpha=0.5, savefilename=None, gaussianwh=600, gaussiansd=None):
...
parser.add_argument('-n', '--n-gaussian-matrix', type=int, default='600', required=False, help='width and height of gaussian matrix')
...
mode = 'train'
video_dir = 'combined/'
pickle_dir = op.join('processed/', mode)

filenames = os.listdir(pickle_dir)
filenames.sort()
filenames = filenames
if True:
    for idx in tqdm(range(0, len(filenames))):
        ...
        filename = op.join(pickle_dir, filename)
        with open(filename, "rb") as f:
            pickle_file = pickle.load(f)
            
            # random_one_img is one frame of the video
            img = pickle_file['img']
            bbox = pickle_file['bbox']
            xyxy = bbox_xywh2xyxy(np.array(bbox))
            ...
            video_path = op.dirname(op.join(video_dir, img_path))
            img_name = op.basename(op.join(video_dir, img_path))
            img_data = cv2.imread(op.join(video_dir, img_path))
            
            img_h, img_w = img_data.shape[:2]
            # img_h, img_w = 10, 10
            print(video_path, img_w, img_h)
            # gaze_data = [(int((xyxy[0]+xyxy[2])/2*img_w), int((xyxy[1]+xyxy[3])/2*img_h), 1)]
            
            test_flip = False
            if test_flip:
                img_data = cv2.flip(img_data, 0)
                xyxy = np.array([xyxy[0], 1-xyxy[3], xyxy[2], 1-xyxy[1]])

            test_crop = False
            if test_crop:
                im_h, im_w, im_c = img_data.shape
                h, w = [500, 500]   # cropped size
                import random
                y1 = random.randint(0, im_h - h)
                x1 = random.randint(0, im_w - w)
                img_data = img_data[y1:y1+h, x1:x1+w]             

                max_size = torch.as_tensor([w, h], dtype=torch.float32)
                raw_bbox = torch.as_tensor([xyxy[0]*im_w, xyxy[1]*im_h, xyxy[2]*im_w, xyxy[3]*im_h])
                ...
                xyxy = cropped_boxes.reshape(4)
                xyxy = torch.as_tensor([xyxy[0]/im_w, xyxy[1]/im_h, xyxy[2]/im_w, xyxy[3]/im_h])
            ...
            cv2.imwrite(f"{idx}_0.jpg", img_data)

            # if os.path.exists(op.join(video_path, 'fake_saliency_my')) and os.path.exists(op.join(video_path, 'fake_saliency_my', img_name)):
            #     try:
            #         cv2.imread(op.join(video_path, 'fake_saliency_my', img_name))
            #         continue
            #     except:
            #         pass
                
            
            # draw_heatmap(gaze_data, (img_w, img_h), alpha=alpha, savefilename=op.join(f"{idx}_1.jpg"), imagefile=op.join(video_dir, img_path), gaussianwh=ngaussian, gaussiansd=sd)
